...
async fn main() {
    ...
    loop {
        ...
        for e in entities.iter_mut() {
            ...
            if e.pos.inner[0] < 0.0 || e.pos.inner[0] > screen_width() {
                e.vel.inner[0] *= -1.0;
            }
            if e.pos.inner[1] < 0.0 || e.pos.inner[1] > screen_height() {
                e.vel.inner[1] *= -1.0;
            }
        }
        if !entities.is_empty() {
            ...
            tree.nodes.clear();
            tree.root = None;
            let build_data: Vec<(Vector<f32, 2>, usize)> =
                entities.iter().map(|e| (e.pos, e.id)).collect();
            let start = get_time();
            ...
            draw_text(
                &format!("Build Time: {:.2}ms", elapsed * 1000.0),
                20.0,
                60.0,
                30.0,
                GREEN,
        ...
        draw_text(
            "Hold Left Click to spawn points",
            20.0,
            screen_height() - 20.0,
            20.0,
            GRAY,
